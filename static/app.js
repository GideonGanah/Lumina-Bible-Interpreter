...
async function runInterpretation(reference) {
  ...
  try {
    ...
    if (data.error === 'OLLAMA_OFFLINE') {
      ...
      document.getElementById('study-reference').textContent  = 'Ollama not reachable';
      document.getElementById('study-status').textContent     = 'Start Ollama and retry';
      showToast('🦙 Ollama is offline. Run "ollama serve" in a terminal then retry.', 8000);
      if (data.translations_available && Object.keys(data.translations_available).length) {
        const count = Object.keys(data.translations_available).length;
        showToast(`✓ Bible text fetched in ${count} versions — only AI analysis needs Ollama.`, 7000);
      }
      return;
    }

    // ── Handle Gemini rate limit (legacy) ──
    if (data.error === 'RATE_LIMIT') {
      document.getElementById('study-loading').style.display = 'none';
      document.getElementById('study-welcome').style.display = 'flex';
      showToast('⏳ API quota reached. Wait ~1 minute then try again.', 7000);
      document.getElementById('study-reference').textContent = 'Quota limit reached';
      document.getElementById('study-status').textContent    = 'Please wait ~1 minute';
      return;
    }
